 ...
 * @returns {boolean}
 */
export function tryPickUp(e, cursor, store, view, inv, item, deltaCoordX = 0, deltaCoordY = 0) {
    if (!item) {
        return false;
    }
 ...
 * @returns {boolean}
 */
export function tryMergeItems(cursor, store, prevInvId, prevItem, heldInvId, heldItem, mergable, shiftKey) {
 ...
 */
export function tryDropPartialItem(store, heldInvId, toInvId, heldItem, mergable, shiftKey, targetCoordX, targetCoordY) {
 ...
 */
export function getDeltaCoords(inv, itemId, clientX, clientY, gridUnit, containerElement) {
