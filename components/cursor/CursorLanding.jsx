...
export default function LandingCursor() {
    ...
    let [ valid, nextCoordX, nextCoordY, width, height ] = getLanding(cursor, store);
    if (typeof valid !== 'boolean') {
        return null;
    }
    let gridUnit = cursor.gridUnit;
    let screenX = nextCoordX * gridUnit - cursor.getCursorWorldX() + cursor.getCursorScreenX();
    let screenY = nextCoordY * gridUnit - cursor.getCursorWorldY() + cursor.getCursorScreenY();
    return (
        <div className={`${styles.landing} ${valid && styles.valid}`}
            style={{
                // @ts-ignore
                '--cursor-x': `${screenX}px`,
                '--cursor-y': `${screenY}px`,
...
export function getLanding(cursor, store) {
    ...
    let nextCoords = [0, 0];
    let result = findEmptyViewArea(nextCoords, store, coordX, coordY, width, height);
    if (!result) {
